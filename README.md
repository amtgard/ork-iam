...
| 2.x (current) | `main` | `"amtgard/ork-iam": "^2.1"` | `v2.1.0` |

Pin `^1.4` for IDP and existing integrators on the 1.x API. Use `^2.1` for the current ontology API (`ServiceCatalog`, `ornSegmentSchema()`, …).
...
### Claim and policy composition (v2.1.0)
...
$claim = ClaimBuilder::forPrefix(ServiceCatalog::Attendance)
    ...
    ->segment(ServiceCatalog::Kingdom, 42)
...
Upgrading from 1.4? See [docs/MIGRATION-2.0.md](docs/MIGRATION-2.0.md#upgrading-from-14-to-21-claim-composition).
